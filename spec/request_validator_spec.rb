...
describe 'request_validator' do
  it 'should return if a request is valid or not' do
    ...
    expect(request_validator.validate("Ask player2 for a J", player1_name, names)).to eq(['player2', 'J'])
    expect(request_validator.validate("Ask player1 for a 2", player1_name, names)).to eq(false)
    expect(request_validator.validate("Ask player2 for a 2", player1_name, names)).to eq(['player2', 2])
